...
# Usage: scripts/run_censorship_judge.sh RESPONSES_JSONL [--limit N]
set -euo pipefail
cd "$(dirname "$0")/.."
RESP=$1; shift || true
...
echo "log: $LOG"
PYTHONPATH="$SP" $UVPY src/censorship_eval/judge_responses.py configs/censorship_eval.yaml "$RESP" "$@" 2>&1 | grep --line-buffered -v torchao | tee "$LOG"
