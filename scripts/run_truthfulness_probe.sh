...
# Usage: [CFG=configs/truthfulness_probe_sweep_gemma4_12b.yaml] scripts/run_truthfulness_probe.sh [GPU_INDEX] [--limit N]
set -euo pipefail
cd "$(dirname "$0")/.."
GPU=${1:-1}; shift || true
CFG=${CFG:-configs/truthfulness_probe.yaml}
...
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=$GPU PYTHONPATH="$SP" \
  $UVPY src/truthfulness_probe/sweep.py $CFG "$@" 2>&1 | grep --line-buffered -v torchao | tee "$LOG"
